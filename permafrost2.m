...
if eq>1
    m_lossCO2 = integral(@(z) rateCO2(tempgrad(eq, 9.75, 104, 201.73, 407.4, 0.6, 1875, z)).*cdensity(z), Z_0, 0); %carbon released in unit area as CO2
    m_lossCH4 = integral(@(z) rateCH4(tempgrad(eq, 9.75, 104, 201.73, 407.4, 0.6, 1875, z)).*cdensity(z), Z_0, 0); %carbon released in unit area as CH4
else 
    m_lossCO2 = 0;
    m_lossCH4 = 0;
end

m_totalCO2 = m_lossCO2.*A %mass per unit area times total arctic area 
m_totalCH4 = m_lossCH4.*A %mass per unit area times total arctic area 
